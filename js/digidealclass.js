class DigiDeal {
		...
		addDestinations(object,value) {
			if(typeof value == 'undefined') {
				// probably and object with multiple destinations or an array
				if(Array.isArray(object)) {
					for(var i in object) {
						this.setDestinations(object[i]);
					}
				} else if (typeof object == 'object') {
					for(var i in object) {
						this.setDestinations(i,object[i]) 
					}
				} else {
					fail('Invalid input');
				}
				...
				if(typeof object == 'string') {
					...
					this._addDestination(object,value);
		...
		_addDestination(address,amount) {
			amount = parseInt(amount);
			if(this.validAmount(amount) && digibyte.Address.isValid(address)) {
				
				if(typeof this.destinations[address] ==='undefined') {
					this.destinations[address] = amount;
				} else {
					this.destinations[address] +=amount;
				}
			}else{
				
				fail('address or armount is not valid');
			}
			return this;
		...
		getAmount() {
			...
			if( !Object.keys(this.destinations).length == 0) {
				des = this.destinations;
			} else if( !Object.keys(this.fdestinations).length == 0){
				des = this.fdestinations;
			}
			for(var i in des) {
				sum+=des[i];
			}
		...
		getFoundationAmount() {
			if(this.coulance) {
				var foundation = this.getAmount()*0.01 > 70000 ? parseInt(this.getAmount()*0.01):70000;
			} else {
				var foundation = 0;
			}
			// 10 dgb is max
		...
		newPayment(data,pvk) {
			...
			this.fdestinations = this.destinations;
			this.destinations = {};
			...
			this.setStatus('publicKey',this.bufferPublicAddress)
				...
				.setStatus('amount',this.getAmount());
			
			
			
			this.remainingAmount = this.getAmount()+this.fee+this.getFoundationAmount();
				...
				return this.checkTransactions(txids)}).then(
					...
					DGBO.getWalletValue(this.bufferPublicAddress).then(walletval=>{
						...
						if(this.verifyEnd(results)) {
							...
							// not yet done per se
							if(balance >= this.getAmount()+this.fee+this.getFoundationAmount()) {
								// enough
								...
								// TODO
									...
									var needed = this.getAmount()+this.fee+this.getFoundationAmount()-balance;
		...
		verifyEnd(txs) {
			...
			for(var i in txs) {
				...
				for(var j in tx.vout) {
					...
					if(typeof vout.scriptPubKey.addresses !== 'undefined' && vout.scriptPubKey.addresses[0] === this.address && this.unit.fromBTC(vout.value).toSatoshis() >= this.getAmount()) {
			...
			return new Promise((resolve, reject) => {
				DGBO.getWalletValue(this.bufferPublicAddress).then(
					wval=>{
						...
						// fee is for the mainwallet
						destinations = this.fdestinations;
							
						if(wval.balanceSat > this.getAmount()+this.fee+this.getFoundationAmount()) {
							
							// spare money will be returned to the last sender.
							destinations[this.firstpayer]= (wval.balanceSat-(this.getAmount()+this.fee+this.getFoundationAmount()));
